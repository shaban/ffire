...
// GenerateRubyPackage generates a complete Ruby gem package (called from package.go)
func GenerateRubyPackage(config *PackageConfig) error {
	return orchestrateTierBPackage(
		config,
		RubyLayout,
		generateRubyWrapperOrchestrated,
		generateRubyMetadataOrchestrated,
		printRubyInstructions,
	)
}

// generateRubyWrapperOrchestrated adapts the wrapper generation to use PackagePaths
func generateRubyWrapperOrchestrated(config *PackageConfig, paths *PackagePaths) error {
	return generateRubyWrapper(config, paths.Root)
}

// generateRubyMetadataOrchestrated generates all Ruby package metadata files
func generateRubyMetadataOrchestrated(config *PackageConfig, paths *PackagePaths) error {
	if err := generateRubyGemspec(config, paths.Root); err != nil {
		return err
	}
	if err := generateRubyGemfile(config, paths.Root); err != nil {
		return err
	}
	if err := generateRubyReadme(config, paths.Root); err != nil {
		return err
	}
	return nil
}

// printRubyInstructions prints installation and usage instructions
func printRubyInstructions(config *PackageConfig, paths *PackagePaths) {
