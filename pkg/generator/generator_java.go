...
func generateJavaWrapper(config *PackageConfig, javaDir string) error {
	...
	if err := generateJavaNativeLoader(javaDir, packageName, config); err != nil {
		return err
	}
...
func generateJavaNativeLoader(javaDir string, packageName string, config *PackageConfig) error {
	buf := &bytes.Buffer{}
	...
	fmt.Fprintf(buf, "            System.loadLibrary(\"%s\");\n", config.Schema.Package)
...
- Java 8 or higher
- Maven 3.x
- Native library (lib%s.so/dylib or %s.dll) must be accessible

## Installation

### Build with Maven

`, config.Namespace, config.Schema.Package, config.Schema.Package, config.Schema.Package)
