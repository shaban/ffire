...
class Program
{
    ...
    static void Main()
    {
        ...
        Console.WriteLine("\nRunning Approach 3 benchmark (correct measurement: pure encode + pure decode)...");

        // Encode
        byte[] encoded = Approach3.Encode(plugins);
        Console.WriteLine($"Encoded size: {encoded.Length} bytes");

        // Warmup
        for (int i = 0; i < 10000; i++)
        {
            byte[] data = Approach3.Encode(plugins);
            Plugin[] decoded = Approach3.Decode(data);
        }

        // Benchmark ENCODE (pure encode from pre-existing object)
        int encodeIterations = 100000;
        ...
        swEncode.Stop();
        long encodeNs = (swEncode.ElapsedMilliseconds * 1_000_000) / encodeIterations;

        // Benchmark DECODE (pure decode to new object)
        int decodeIterations = 100000;
        var swDecode = Stopwatch.StartNew();
        for (int i = 0; i < decodeIterations; i++)
        {
            Plugin[] decoded = Approach3.Decode(encoded);
        }
        swDecode.Stop();
        long decodeNs = (swDecode.ElapsedMilliseconds * 1_000_000) / decodeIterations;

        long totalNs = encodeNs + decodeNs;

        Console.WriteLine($"\nApproach 3 Results:");
        Console.WriteLine($"  Encode: {encodeNs:N0} ns/op");
        Console.WriteLine($"  Decode: {decodeNs:N0} ns/op");
        Console.WriteLine($"  Total:  {totalNs:N0} ns/op");
    }
}
