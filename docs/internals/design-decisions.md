...
## Decision 0: Bundled Deployment (No Symbol Versioning)

**Date**: November 8, 2025

### Decision
Each language package bundles its own dylib. Symbol versioning will NOT be implemented.

### Rationale
- **Isolation**: Each package is self-contained
- **No conflicts**: Different apps can use different ffire versions
- **Simple deployment**: pip install, pub get, etc. just work
- **No runtime checks**: No need to detect library version

### Deployment Model
```
python_package/
  myschema/
    lib/libmyschema.dylib    # Bundled
    __init__.py
...
- ❌ Shared library approach across applications

### Impact
✅ **Gained**: Zero version conflicts  
✅ **Gained**: No symbol versioning complexity  
✅ **Gained**: Simple packaging  
❌ **Lost**: Cannot share single dylib across apps  
✅ **Acceptable**: Disk space is cheap, version conflicts are expensive

**This decision is FINAL.** Do not revisit unless deployment model changes.
