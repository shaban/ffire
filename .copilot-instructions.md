...
language_paradigms:
  ...
  cpp:
    ...
    root_type:
      ...
      status: "⚠️ CURRENTLY BROKEN: Generates 'struct Config' without suffix"
  ...
  dart:
    ...
    root_type:
      ...
      status: "⚠️ CURRENTLY BROKEN: Generates 'class Config' without suffix"
  ...
  python:
    tier: "B"
    binding: "ctypes (Tier B) or pybind11 (Tier A)"
    status: "⚠️ UNKNOWN: Likely has same Message suffix bug as Dart"
...
validation:
  ...
  known_architectural_violations:
    cpp_generator:
      ...
      issue: "Generates 'struct Config' without Message suffix"
      should_be: "struct ConfigMessage"
      status: "needs_fix"
      
    dart_generator:
      file: "pkg/generator/generator_dart.go"
      issue: "Generates 'class Config' without Message suffix"
      should_be: "class ConfigMessage"
      status: "needs_fix"
      
    python_generator:
      file: "pkg/generator/generator_python_*.go"
      issue: "Unknown, likely has same bug as Dart"
...
workflows:
  ...
  debugging_benchmark_failure:
    decision_tree:
      ...
      - "  YES → Benchmark harness might be wrong, investigate"
...
**⚠️ Known Violations:**
- C++ generator: Missing Message suffix on structs
- Dart generator: Missing Message suffix on classes
- Python generator: Unknown, needs investigation

**Next Steps:**
1. Fix C++ generator to use `struct {Name}Message`
2. Fix Dart generator to use `class {Name}Message`
3. Investigate Python generator
4. Update docs/ to match this YAML
